...
> Rizzo, U., Rentocchini, F., Seeber, M. and Ramaciotti, L. (2025), Beyond the Badge of Honour: The Effect of the Academic Excellence Initiative on Staff Recruitment in Italy, _Oxford Economic Papers_, forthcoming
...
- There is no need to download the stata packages, as they are already included in `stata_packages`. Running `scripts/00_main.do` is sufficient to generate the estimates. Since several of these packages might have been updated recently, it is advisable to check for updates by changing `global downloads 0` to `global downloads 1` in `scripts/00_main.do`. This will automatically update all packages.
- The package was run on three different instances: 1) A virtual machine with Stata MP version 19.5, Windows 11 Enterprise 64-bit, PC (64-bit x86-64) with 8 processors, AMD EPYC 7502 32-Core Processor (2.50 GHz), RAM 32 GB and completed in 6 hours and 16 minutes; 2) Stata MP version 19.5, Windows 11 Enterprise 64-bit, PC (64-bit x86-64) with 8 processors, Intel(R) Core(TM) i7-8565U CPU @ 1.80GHz (1.99 GHz), RAM 16 GB and completed in 4 hours and 50 minutes; 3) Stata SE version 19.5, Windows 11 Pro, PC (x64) with 12th Gen Intel(R) Core(TM) i7-1270P processor (2.20 GHz), RAM 32 GB, and completed in 1 hour and 37 minutes. No specific hardware or software requirements are needed, although older versions of Stata and less powerful PCs may encounter issues when running the full code.
