...
preserve
	...
	* All outcomes (combined)
	grc1leg "${temp_path}/gr_new_position.gph" ///
		...
		"to control units to construct the synthetic control that minimize the differences with the treated unit during the pre-treatment period. Standard errors are based on ${reps} bootstrap replications. ", span size(.15cm))
...
esttab matrix(A, fmt(3 3 3 3 3 3)) using ///
    ...
    addnotes("Notes: this table displays results from the synthetic difference-in-difference estimator (Arkhangelsky et al., 2021), based on `r(N)' department-year observations using 2014-2020 data. Estimates include controls for the number of employees at t-1, the number of staff transferred between departments, university income linked to the VQR, province-level per-capita value added and provincial unemployment rate. Standard errors are based on ${reps} bootstrap replications.")
